...
coverage: clean
	...
	./build/tests/unit_tests --seed 0x526af79e
	# GCC / lcov branch
	@if [ "$(shell $(CC) -v 2>&1 | grep -c "gcc")" -gt 0 ]; then \
       	...
       	LLVM_PROFILE_FILE="coverage.profraw" ./build/tests/unit_tests; \
       	llvm-profdata merge -sparse coverage.profraw -o coverage.profdata; \
       	llvm-cov report ./build/tests/unit_tests -instr-profile=coverage.profdata; \
	fi
